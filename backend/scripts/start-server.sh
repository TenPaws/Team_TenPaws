...
# 업로드 디렉토리 설정
echo "업로드 디렉토리 설정..."
mkdir -p /home/ubuntu/tenpaws-server/uploads
chmod 777 /home/ubuntu/tenpaws-server/uploads || {
...
# ECR 이미지 pull 시도
echo "도커 이미지 가져오기 시도..."
if ! docker pull 761018890747.dkr.ecr.ap-northeast-2.amazonaws.com/tenpaws-server:latest; then
...
docker inspect tenpaws-server_nginx_1 || {
